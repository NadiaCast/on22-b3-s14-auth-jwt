...
const SECRET = process.eventNames.SECRET;

const findAllGames = async (req, res) => {
  try {
    const authHeader = req.get("authorization")
    if (!authHeader) {
      return res.status(401).send("informações de autorização não passada")
    }
...
const findGameById = async (req, res) => {
  try {
    const authHeader = req.get("authorization")
    if (!authHeader) {
      return res.status(401).send("Não tem nada aqui")
    }
...
const addNewGame = async (req, res) => {
  try {
    const authHeader = req.get("authorization")
    if (!authHeader) {
      return res.status(401).send("Nã há cabeçalho quirida")
    }
    ...
    jwt.verify(token, SECRET, async function (erro) {
      if (erro) {
        return res.status(403).send("Você não pode acessar")
      }
      const {
        consoleId,
        name,
...
const updateGame = async (req, res) => {
  try {
    const authHeader = req.get("authorization")
    if (!authHeader) {
      return res.status(401).send("Não tem autorização")
    }
...
const deleteGame = async (req, res) => {
  try {
    const authHeader = req.get("authorization")
    if (!authHeader) {
      return res.status(401).send("Esqueceu a autorização")
    }
